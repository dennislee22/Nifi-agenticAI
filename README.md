...
These are passed to an `InvokeHTTP` processor, which sends them to an Cloudera AI (CAI) platform acting as the Reasoning engine. NiFi’s `EvaluateJsonPath` processor extracts the sentiment label and score from the JSON and promotes them to attributes. 
