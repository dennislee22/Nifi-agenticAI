...
In a typical **Agentic AI** solution, the initial phase begins with `Perception`, which focuses on ingesting and curating data. During this stage, raw data from diverse sources is gathered and converted into a structured format for the `Reasoning` engine to digest and infer accordingly.
...
NiFi’s `EvaluateJsonPath` processor extracts the sentiment label and score from the JSON and promotes them to attributes. 
